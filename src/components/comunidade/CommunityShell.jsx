...
function Agenda({ mediator = false }) {
  const { rituais = [], clubes, saveRitual } = useCommunity();
  const mine = clubes.filter((club) => club.membros.includes("me"));
  const first = rituais.find((item) => mediator || mine.some((club) => club.id === item.clubeId));
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(first || { id: "passaporte", clubeId: mine[0]?.id || clubes[0]?.id || "", nome: "", recorrencia: "", descricao: "", proximo: "", local: "" });
  const club = clubes.find((item) => item.id === first?.clubeId);
  return <><Title eyebrow="Rituais da comunidade" title="Agenda">Encontros que voltam a acontecer e ajudam os clubes a criar identidade, memória e participação.</Title>{first && <Card className="co-ritual-card"><div className="co-row"><span className="co-card-icon"><Compass size={19} /></span><span className="co-tag">Ritual do meu clube</span></div><p className="co-eyebrow">{club?.nome}</p><h3>{first.nome}</h3><p>{first.descricao}</p><div className="co-ritual-details"><span><CalendarDays size={15} /><strong>{first.recorrencia}</strong></span><span><MapPin size={15} /><strong>{first.local}</strong></span></div><p className="co-note">Próximo encontro · {first.proximo}</p>{mediator && <><p className="co-small co-muted">Você organiza o acesso ao espaço e publica a agenda. O embaixador mobiliza o clube.</p><Button secondary onClick={() => { setDraft(first); setEditing(!editing); }}><Pencil size={15} /> Editar ritual</Button></>}</Card>}{mediator && editing && <Card><h3>Editar e lançar na agenda</h3><form onSubmit={(event) => { event.preventDefault(); saveRitual(draft); setEditing(false); }}><label className="co-field">Clube<select value={draft.clubeId} onChange={(event) => setDraft({ ...draft, clubeId: event.target.value })}>{clubes.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></label><Field label="Nome do ritual" value={draft.nome} onChange={(nome) => setDraft({ ...draft, nome })} required /><Field label="Quando acontece" value={draft.recorrencia} onChange={(recorrencia) => setDraft({ ...draft, recorrencia })} required /><Field label="Próximo encontro" value={draft.proximo} onChange={(proximo) => setDraft({ ...draft, proximo })} required /><Field label="Local" value={draft.local} onChange={(local) => setDraft({ ...draft, local })} required /><Field label="O que vai acontecer" value={draft.descricao} onChange={(descricao) => setDraft({ ...draft, descricao })} multiline required /><Button type="submit" disabled={!draft.nome.trim() || !draft.recorrencia.trim() || !draft.proximo.trim() || !draft.local.trim()}>Publicar na agenda dos alunos</Button></form></Card>}<Card><h3>O que é um ritual?</h3><p className="co-muted">É um encontro recorrente criado pela comunidade. Diferente de uma missão, ele volta à agenda e pode receber novos temas, convidados e participantes.</p></Card></>;
}
