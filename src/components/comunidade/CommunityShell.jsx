...
import { Home, Users, Hexagon, UserRound, ArrowLeft, Sun, Moon, Bell, Pencil, Plus, MessageCircle, Heart, Camera, Sparkles, BadgeCheck, School, MapPin, RotateCcw, WandSparkles, Route, PhoneOff, TimerReset, CalendarDays, Compass, ChevronLeft, ChevronRight, BookOpen, Music2, Bot, Palette, Gamepad2, Film, Drama, Leaf, Handshake, Volleyball } from "lucide-react";
import { CRITERIOS, INTERESSES, TURMAS, TURNOS, profileLabel, useCommunity, timeAgo } from "@/state/CommunityState";
import BeeMascot from "@/components/colmeia/BeeMascot";
import { useSearchParams } from "react-router-dom";
import { motion, AnimatePresence, useReducedMotion } from "framer-motion";
import { LOGO_COLMEIA_MARK } from "@/components/colmeia/SiteHeader";
...
const CLUB_ICONS = { sparkles: Sparkles, book: BookOpen, music: Music2, robot: Bot, art: Palette, games: Gamepad2, cinema: Film, theater: Drama, leaf: Leaf, handshake: Handshake, sports: Volleyball, compass: Compass };
const CLUB_ICON_OPTIONS = [["sparkles", "Ideias"], ["book", "Leitura"], ["music", "Música"], ["robot", "Robótica"], ["art", "Artes"], ["games", "Jogos"], ["cinema", "Cinema"], ["theater", "Teatro"], ["leaf", "Natureza"], ["handshake", "Acolhimento"], ["sports", "Esportes"], ["compass", "Descobertas"]];
function ClubIcon({ id, size = 21 }) { const Icon = CLUB_ICONS[id] || Sparkles; return <Icon size={size} aria-hidden="true" />; }

export default function CommunityShell() {
  const { dark, toggleDark, resetDemo, storageError, avisos, pedidos, alertasMediador } = useCommunity();
...
function Feed({ openClub }) {
  ...
  const interests = (Array.isArray(profile.interesses) ? profile.interesses : String(profile.interesses || "").split(",")).flatMap((item) => normalize(item).split(/[^a-z0-9]+/)).filter((word) => word.length > 3);
  const score = (post) => {
  ...
  return <><Title eyebrow="Saberes que circulam" title={`Oi, ${profile.nome}!`}>O que a escola está criando hoje?</Title>
    ...
    <div className="co-club-strip" aria-label="Meus clubes">{mine.map((c, i) => <button key={c.id} onClick={() => openClub(c.id, c.comunidadeId)}><span className={`co-club-orb orb-${i % 3}`}><ClubIcon id={c.icon} /></span><span>{c.nome}<small>{c.turmas.length} turmas</small></span></button>)}</div>
    <Composer />
...
function Composer({ clubeId }) {
  ...
  return <Card><button className="co-compose-toggle" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}><span className="co-avatar"><Sparkles size={19} /></span><span>Compartilhe uma descoberta…</span><Plus size={16} /></button>{sent && !expanded && <p role="status">Descoberta compartilhada!</p>}{expanded && <form onSubmit={(e) => { e.preventDefault(); const result = createPost(clubeId || selected, text, photo); if (!result?.ok) { setModeration(result?.message || "Revise a publicação."); return; } setModeration(""); setPhoto(null); setText(""); setSent(true); setExpanded(false); }}>
    ...
    <Field label="Sua descoberta" value={text} onChange={(t) => { setText(t); setSent(false); setModeration(""); }} multiline />
    ...
    <p className="co-small co-muted">Visível no clube e no feed da comunidade escolar. A moderação assistida revisa o texto antes de publicar.</p>{moderation && <p role="alert" className="co-moderation-note">{moderation}</p>}<Button type="submit" disabled={!text.trim() && !photo}>Compartilhar saber</Button>{sent && <p role="status">Descoberta compartilhada!</p>}
  </form>}</Card>;
}
...
function Clubs({ openClub }) {
  ...
  return <><Title eyebrow="A escola é a comunidade" title="Clubes">Encontre interesses em comum e construa com estudantes de outras turmas.</Title><div className="co-tabs">{[["minhas", "Meus clubes"], ["descobrir", "Descobrir"]].map(([id, label]) => <button key={id} aria-pressed={filter === id} onClick={() => setFilter(id)}>{label}</button>)}</div>
    ...
    <Button secondary onClick={() => setCreating(!creating)}><Plus size={16} /> Propor um clube</Button>{creating && <Card><form onSubmit={(e) => { e.preventDefault(); createClub("convivencia", name, description, icon); setName(""); setDescription(""); setIcon("sparkles"); setCreating(false); }}><Field label="Nome do clube" value={name} onChange={setName} required /><Field label="O que vocês querem construir juntos?" value={description} onChange={setDescription} multiline required /><fieldset className="co-icon-picker"><legend>Escolha um símbolo para o clube</legend>{CLUB_ICON_OPTIONS.map(([id, label]) => <button type="button" key={id} aria-pressed={icon === id} onClick={() => setIcon(id)}><ClubIcon id={id} /><span>{label}</span></button>)}</fieldset><p className="co-small">Você começa como embaixador e pode convidar colegas de outras turmas.</p><Button type="submit" disabled={!name.trim() || !description.trim()}>Criar clube</Button></form></Card>}
  </>;
}
function ClubCard({ club, openClub }) { return <Card><span className="co-club-card-icon"><ClubIcon id={club.icon} size={24} /></span><h3>{club.nome}</h3><p className="co-muted">{club.descricao}</p><p className="co-small">{club.membros.length} participantes · {club.turmas.join(" · ")}</p><span className={club.embaixador === "me" ? "co-ambassador-tag" : "co-tag"}>{club.embaixador === "me" ? "Você é embaixador" : club.membros.includes("me") ? "Você participa" : "Inscrições abertas"}</span><Button secondary onClick={() => openClub(club.id)}>Abrir clube</Button></Card>; }
...
function Missions({ openMission }) {
  ...
  const mine = missoes.filter((m) => m.escopo === "turma" ? m.turma === profile.ano : clubes.some((c) => c.id === m.clubeId && c.membros.includes("me"))).sort((a, b) => Number(a.status === "concluida") - Number(b.status === "concluida") || a.prazo - b.prazo);
...
function Agenda({ mediator = false }) {
  ...
  const visible = rituais.filter((item) => mediator || item.escopo === "turma" ? mediator || item.turma === profile.ano : mine.some((club) => club.id === item.clubeId)).sort((a, b) => `${a.data}${a.hora}`.localeCompare(`${b.data}${b.hora}`));
...
function Profile() {
  ...
  const selectedInterests = Array.isArray(draft.interesses) ? draft.interesses : [];
  const toggleInterest = (interest) => setDraft({ ...draft, interesses: selectedInterests.includes(interest) ? selectedInterests.filter((item) => item !== interest) : [...selectedInterests, interest] });
  return <><Title eyebrow="Seu lugar na Colmeia" title="Perfil" /><Card><button className="co-profile-bee" aria-label="Abrir Minha Abelha" onClick={() => setAvatar(!avatar)}><BeeMascot size={80} equipped={profile.equipped} /></button><h3>{profile.nome}</h3><p>{profileLabel(profile)}</p><p className="co-small co-muted">{(profile.interesses || []).join(" · ")}</p><Button secondary onClick={() => { setDraft(profile); setEdit(!edit); setSaved(false); }}><Pencil size={15} /> Editar perfil</Button>{saved && <p role="status">Perfil salvo neste navegador.</p>}</Card>
    {edit && <Card><form onSubmit={(e) => { e.preventDefault(); saveProfile({ ...draft, nome: draft.nome.trim() }); setEdit(false); setSaved(true); }}><Field label="Nome" value={draft.nome} onChange={(nome) => setDraft({ ...draft, nome })} required /><label className="co-field">Ano escolar<select value={draft.ano} onChange={(e) => setDraft({ ...draft, ano: e.target.value })}>{TURMAS.map((t) => <option key={t}>{t}</option>)}</select></label><label className="co-field">Turno<select value={draft.turno} onChange={(e) => setDraft({ ...draft, turno: e.target.value })}>{TURNOS.map((turno) => <option key={turno}>{turno}</option>)}</select></label><details className="co-interest-picker"><summary>{selectedInterests.length ? `${selectedInterests.length} interesses selecionados` : "Escolha seus interesses"}</summary><div>{INTERESSES.map((interest) => <label key={interest}><input type="checkbox" checked={selectedInterests.includes(interest)} onChange={() => toggleInterest(interest)} /><span>{interest}</span></label>)}</div></details><Button type="submit" disabled={!draft.nome.trim() || !selectedInterests.length}>Salvar perfil</Button></form></Card>}
...
function MediatorNotifications() {
  const { alertasMediador, pedidos } = useCommunity();
  return <>{alertasMediador.map((alert) => <Card key={alert.id} className="co-alert-card"><span className="co-card-icon"><Camera size={18} /></span><h3>{alert.texto}</h3><p className="co-small co-muted">{alert.detalhe}</p><span className="co-small">{timeAgo(alert.criadoEm)}</span></Card>)}<h3 className="co-section-title">Pedidos de apoio</h3>{pedidos.length ? pedidos.map((pedido) => <SupportCard key={pedido.id} pedido={pedido} />) : !alertasMediador.length ? <Card>Nenhuma notificação agora.</Card> : <Card>Nenhum pedido de apoio agora.</Card>}</>;
}
...
function Mediator({ tab }) {
  const { clubes, missoes, posts, alertasMediador } = useCommunity();
  ...
  return <><Title eyebrow="Presença que apoia" title="Apoiar a Colmeia">Os alunos conduzem os clubes. Você ajuda com escuta, inclusão e os recursos da escola.</Title><div className="co-stats co-stats-three"><Card><strong>{clubes.length}</strong><br />clubes</Card><Card><strong>{missoes.filter((m) => m.status === "ativa").length}</strong><br />missões ativas</Card><Card><strong>{activatedSpaces.length}</strong><br />espaços ativados</Card></div>
    ...
    {section === "apoio" && <>{alertasMediador.length > 0 && <><h3>Novas evidências</h3>{alertasMediador.slice(0, 3).map((alert) => <Card key={alert.id} className="co-alert-card"><Camera size={18} /><strong>{alert.texto}</strong><p className="co-small co-muted">{alert.detalhe}</p></Card>)}</>}<h3>Pedidos de apoio</h3><Support /><h3 className="co-section-title">Feed da comunidade escolar</h3>{posts.slice(0, 3).map((p) => <Card key={p.id}><ClubLabel clubeId={p.clubeId} /><p><strong>{p.autor}</strong></p><p>{p.texto}</p></Card>)}</>}
...
const DEMO_STUDENTS = [
  { participante: "Bia", turma: "7º ano", interesse: "acolhimento" },
  { participante: "Lia", turma: "6º ano | Manhã", interesse: "expressão" },
  { participante: "Noa", turma: "8º ano | Tarde", interesse: "escuta" },
  { participante: "Ravi", turma: "9º ano | Integral", interesse: "organização" },
  { participante: "Maya", turma: "6º ano | Tarde", interesse: "natureza" },
  { participante: "Caio", turma: "8º ano | Integral", interesse: "tecnologia" },
  { participante: "Sol", turma: "9º ano | Integral", interesse: "arte" },
  { participante: "Ian", turma: "7º ano | Tarde", interesse: "movimento" },
];
const THEMES = {
...
function MediatorMissionComposer({ done }) {
  ...
  const [target, setTarget] = useState("7º ano");
