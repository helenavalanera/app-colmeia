...
import { Home, Users, Hexagon, UserRound, ArrowLeft, Sun, Moon, Bell, Pencil, Plus, MessageCircle, Heart, Camera, Sparkles, BadgeCheck, School, MapPin, RotateCcw, WandSparkles, Route } from "lucide-react";
...
const AI_FAVO_TEMPLATES = [
  { titulo: "Observar", pergunta: "Observe uma situação ligada à missão. Que barreira ou possibilidade costuma passar despercebida?", pista: "Procure um espaço de passagem onde encontros inesperados costumam acontecer.", local: "Pátio coberto" },
  { titulo: "Escutar", pergunta: "Converse com alguém que usa este espaço. O que faria essa pessoa se sentir ouvida e incluída?", pista: "Siga até o lugar onde muitas histórias esperam para ser abertas.", local: "Biblioteca" },
  { titulo: "Imaginar", pergunta: "Encontre um detalhe do ambiente que poderia virar um convite para participar. Que mudança você propõe?", pista: "Vá ao lugar onde a turma se reúne antes de começar uma atividade.", local: "Corredor das salas" },
  { titulo: "Conectar", pergunta: "Relacione uma necessidade a uma ação possível. Quem poderia ajudar a tornar essa ideia parte da escola?", pista: "Procure o espaço onde estudantes de anos diferentes se encontram no intervalo.", local: "Quadra" },
];
function createAiDistribution(title, objective) {
  return AI_FAVO_TEMPLATES.map((template, index) => ({ ...template, ...AI_STUDENTS[index], pergunta: `${template.pergunta} Conecte sua descoberta a “${title}” e ao objetivo: ${objective}` }));
}
...
function MediatorMissionComposer({ done }) {
  ...
  function invalidateDistribution(update) { update(); setDistribution(null); }
  function toggleCriterion(item) { setCriteria((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item]); setDistribution(null); }
  return <Card><span className="co-ai-label"><WandSparkles size={13} /> Assistente de missão</span><h3>Lançar uma missão</h3><p className="co-small co-muted">Diga a experiência e a intenção psicossocial. A IA propõe o match entre estudantes, quatro favos complementares e pistas para ativar espaços da escola.</p><form onSubmit={(e) => { e.preventDefault(); if (!distribution) return; const clubeId = scope === "clube" ? target : fallbackClub?.id; createMission({ escopo: scope, turma: scope === "turma" ? target : undefined, clubeId, titulo: title.trim(), objetivo: objective.trim(), descricao: description.trim(), favo: distribution[0].pergunta, pistaLocal: distribution[0].pista, criterios: criteria, distribuicao: distribution, origemIA: true }); done(); }}><label className="co-field">Enviar para<select value={scope} onChange={(e) => { const next = e.target.value; setScope(next); setTarget(next === "turma" ? "7º B" : clubes[0]?.id || ""); setDistribution(null); }}><option value="turma">Uma turma</option><option value="clube">Um clube</option></select></label><label className="co-field">{scope === "turma" ? "Turma" : "Clube"}<select value={target} onChange={(e) => invalidateDistribution(() => setTarget(e.target.value))}>{scope === "turma" ? TURMAS.map((t) => <option key={t}>{t}</option>) : clubes.map((c) => <option key={c.id} value={c.id}>{c.nome}</option>)}</select></label><Field label="Missão que você quer propor" value={title} onChange={(value) => invalidateDistribution(() => setTitle(value))} required /><Field label="Objetivo da missão" value={objective} onChange={(value) => invalidateDistribution(() => setObjective(value))} multiline required /><Field label="Contexto para a IA" value={description} onChange={(value) => invalidateDistribution(() => setDescription(value))} multiline required /><fieldset className="co-criteria"><legend>Competências gerais da BNCC</legend>{CRITERIOS.map((item) => <label key={item}><input type="checkbox" checked={criteria.includes(item)} onChange={() => toggleCriterion(item)} /> <span>{item}</span></label>)}</fieldset><Button secondary disabled={!title.trim() || !objective.trim() || !description.trim() || !criteria.length} onClick={() => setDistribution(createAiDistribution(title.trim(), objective.trim()))}><WandSparkles size={16} /> {distribution ? "Gerar outra distribuição" : "Gerar distribuição de favos"}</Button>{distribution && <div className="co-ai-plan" role="status"><div className="co-row"><div><span className="co-eyebrow">Proposta da IA</span><h3>Match pronto para revisão</h3></div><BadgeCheck size={24} /></div><p className="co-small">A proposta cruza turmas, interesses e perspectivas. O mediador revisa antes de publicar; os estudantes recebem apenas o próprio favo.</p>{distribution.map((item, index) => <div className="co-ai-favo" key={item.participante}><span>{index + 1}</span><div><strong>{item.participante} · {item.turma}</strong><small>{item.interesse} · {item.titulo}</small><p>{item.pergunta}</p><em><MapPin size={12} /> Pista para {item.local}</em></div></div>)}</div>}<p className="co-note">Demonstração local da integração: a distribuição é gerada no navegador. Na versão conectada, a IA usará perfis autorizados e o mediador continuará responsável pela revisão.</p><Button type="submit" disabled={!target || !distribution}>Revisar e publicar missão</Button></form></Card>;
}
